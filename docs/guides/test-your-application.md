...
Use `ant-sdk v0.12.0` and `ant-node v0.17.1` together for this integration setup, rather than substituting the independently released `antd 0.14.0` or `ant-node 0.20.0`. Keep it separate from newer client installations and complete the isolated test before depending on it.
