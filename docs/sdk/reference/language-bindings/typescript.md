...
Use the TypeScript client library in your Node.js application to store and retrieve data. It sends requests to a local daemon, a separate background service called **antd** that connects to the Autonomi Network. Follow [Start the Local Daemon](../../start-the-local-daemon.md) before connecting. This reference covers the typed REST client.

## Install

Use Node.js 18 or later. Install [@withautonomi/antd](https://www.npmjs.com/package/@withautonomi/antd), which includes TypeScript declarations, in your application directory:

```bash
npm install @withautonomi/antd
```

Installing the client does not install or start antd. Keep the `@withautonomi/` scope: the npm package named `antd` is the unrelated Ant Design library. Examples on this page use `@withautonomi/antd` 0.2 with antd 0.14.

If your project does not already have TypeScript tooling, install these development tools to run the examples:

```bash
npm install --save-dev @types/node tsx typescript
```

Save each example as `app.ts` and run it with `npx tsx app.ts`.
...
For a different port, pass `baseUrl` to `createClient`, or use `RestClient.autoDiscover()` as described in the [JavaScript client](javascript.md#connect-to-antd). The health result does not include peer counts or `write_ready`; use the [REST health endpoint](../rest-api.md#health) for those fields. A successful health check is not a guarantee that an upload will succeed.
...
async function main(): Promise<void> {
  const client = createClient();
  const data: Buffer = await client.dataGetPublic(
...
|------|------|------|
| `health()` | None | `Promise<HealthStatus>` |
| `dataGetPublic(address)` | Public address as a 64-character hexadecimal `string` | `Promise<Buffer>` |
| `dataGet(dataMap)` | Caller-held DataMap as a hexadecimal `string` | `Promise<Buffer>` |
| `dataPutPublic(data)` | `Buffer` | `Promise<DataPutPublicResult>`; retrieve with its `address` |
| `dataPut(data)` | `Buffer` | `Promise<DataPutResult>`; retain its `dataMap` for retrieval |
