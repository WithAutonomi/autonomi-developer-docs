...
- For paid uploads on the default network: access to wallet and payment configuration. See [Prepare a Wallet for Uploads](../how-to-guides/manage-keys.md) and [Use External Signers for Upload Payments](../how-to-guides/use-external-signers.md).
- For a fully local devnet: Python 3.10+ and a sibling `ant-node` checkout if you plan to use `ant dev start`. See [Set Up a Local Network](../how-to-guides/setup-local-network.md).
...
This mode is for the two-phase prepare and finalize upload flow described in [Use External Signers for Upload Payments](../how-to-guides/use-external-signers.md).
