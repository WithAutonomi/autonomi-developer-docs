...
1. Use `ADR-NNNN-short-title.md` names with four-digit numbers. Choose the next unused sequential number after the highest existing ADR; never reuse a number.
2. New prospective ADRs start as `Proposed`. A named decision owner may mark one `Accepted` only after human engineering review; AI tools may not accept ADRs.
