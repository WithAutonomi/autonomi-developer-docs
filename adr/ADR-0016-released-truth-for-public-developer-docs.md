...
- Every default or recommended public installation command uses an immutable version or digest selector, resolves through its documented supported route from a clean environment, and records the exact immutable identity obtained.
...
- Moving a documented convenience alias to a different immutable target blocks its continued recommendation until requalification completes; the alias movement alone cannot establish support.
